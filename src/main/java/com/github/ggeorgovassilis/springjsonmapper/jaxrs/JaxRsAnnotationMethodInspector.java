...
public class JaxRsAnnotationMethodInspector extends BaseAnnotationMethodInspector {
	...
	public UrlMapping inspect(Method method, Object[] args) {
		...
		if (httpMethods.size() > 1)
			throw new MappingDeclarationException("Multiple HTTP methods specified on " + method.toGenericString(),
					method, null, -1);
		if (httpMethods.size() == 1)
			...
			for (Annotation annotation : annotations) {
				if (PathParam.class.isAssignableFrom(annotation.annotationType())) {
					...
					urlMapping.addDescriptor(
							new MethodParameterDescriptor(Type.pathVariable, parameterName, value, method, i));
					parameterFound = true;
				}
